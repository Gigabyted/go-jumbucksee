...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gjbsee-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we're the child process for runGjbsee.
...
// spawns gjbsee with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGjbsee(t *testing.T, args ...string) *testgjbsee {
	tt := &testgjbsee{T: t, Executable: os.Args[0]}
	...
	// Boot "gjbsee". This actually runs the test binary but the init function
...
//
//     gjbsee.expect(`Passphrase: {{.InputLine "password"}}`)
func (tt *testgjbsee) InputLine(s string) string {
	io.WriteString(tt.stdin, s+"\n")
	return ""
}

func (tt *testgjbsee) setTemplateFunc(name string, fn interface{}) {
	if tt.Func == nil {
...
// before matching.
func (tt *testgjbsee) expect(tplsource string) {
...
func (tt *testgjbsee) matchExactOutput(want []byte) error {
...
// after expectRegexp.
func (tt *testgjbsee) expectRegexp(resource string) (*regexp.Regexp, []string) {
	var (
...
// returned.
func (tt *testgjbsee) stderrText() string {
	tt.stderr.mu.Lock()
	defer tt.stderr.mu.Unlock()
	return tt.stderr.buf.String()
}

func (tt *testgjbsee) withKillTimeout(fn func()) {
