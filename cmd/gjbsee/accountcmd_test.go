...
func TestAccountListEmpty(t *testing.T) {
	gjbsee := runGjbsee(t, "account")
	gjbsee.expectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gjbsee := runGjbsee(t, "--datadir", datadir, "account")
	defer gjbsee.expectExit()
...
func TestAccountNew(t *testing.T) {
	gjbsee := runGjbsee(t, "--lightkdf", "account", "new")
	defer gjbsee.expectExit()
	gjbsee.expect(`
...
`)
	gjbsee.expectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gjbsee := runGjbsee(t, "--lightkdf", "wallet", "import", "testdata/guswallet.json")
	defer gjbsee.expectExit()
	gjbsee.expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gjbsee.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if strings.Index(gjbsee.stderrText(), m) == -1 {
...
// https://github.com/jumbucks/go-jumbucksee/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gjbsee.setTemplateFunc("keypath", func(file string) string {
